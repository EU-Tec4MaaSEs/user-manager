...
public class UserController {
     ...
     */
    @Operation(summary = "Re-send activation email for not enabled users", security = @SecurityRequirement(name = "bearerToken"))
    @ApiResponses(value = {
            @ApiResponse(responseCode = "200", description = "Activation email has been successfully sent to user"),
            ...
            @ApiResponse(responseCode = "403", description = "Invalid JWT token attributes | Thrown when some attributes are missing from the JWT Token"),
            @ApiResponse(responseCode = "403", description = "You are unauthorized to request/modify this resource | Thrown when a user attempts to alter a resource that he/she is not authorized to access"),
            @ApiResponse(responseCode = "409", description = "User is already activated. Activation email cannot be resent.")
    })
    @PreAuthorize("hasAnyRole('SUPER_ADMIN', 'ADMIN')")
    ...
    public ResponseEntity<BaseAppResponse<Void>> resendActivationEmail(@AuthenticationPrincipal Jwt jwt, @RequestParam String userId) {
        ...
        if (existingUser.isEnabled())
            return new ResponseEntity<>(
                    BaseAppResponse.error("User is already activated. Activation email cannot be resent."),
                    HttpStatus.CONFLICT
            );
        ...
        String autogeneratedToken = UUID.randomUUID().toString();
        userManagerService.updateActivationToken(userId, autogeneratedToken);

        // Send activation link async
        String activationToken = userId.concat("@").concat(autogeneratedToken);
        String fullName = existingUser.getFirstName() + " " + existingUser.getLastName();
        emailService.sendActivationLink(fullName, existingUser.getEmail(), activationToken);

        return new ResponseEntity<>(
                BaseAppResponse.success(null, "Activation email has been successfully sent to user"),
                HttpStatus.OK
        );
    }
